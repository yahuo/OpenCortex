...
def _run_search_step(
    ...
    glob_hits = core.glob_search(bundle, query_plan.path_globs, kb=kb, allowed_sources=allowed_sources)
    ...
    ast_hits = core.ast_search(bundle, query_plan, kb=kb, allowed_sources=allowed_sources)
    graph_seed_sources = core._dedupe_strings(
        [
            *wiki_seed_sources,
            *core._candidate_sources_from_hits(glob_hits[:3], grep_hits[:3], ast_hits[:3]),
        ]
    )
    ...
    narrowed_vector_scope = wiki_scope | set(core._candidate_sources_from_hits(glob_hits, grep_hits[:4], ast_hits[:4]))
    ...
    vector_hits = core.vector_search(
        bundle,
